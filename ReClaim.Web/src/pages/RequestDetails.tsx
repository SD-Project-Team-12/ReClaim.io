...
export default function RequestDetails() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (data.request?.imageUrls?.length > 0)
          setActiveImage(data.request.imageUrls[0]);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const handleAction = async (
    ...
    const confirmMsg =
      action === "claim"
        ? "Assign this to your route?"
        : `Mark as ${getStatusData(newStatus!).label}?`;
    ...
    try {
      ...
      const res =
        action === "claim"
          ? await claimPickUpRequest(id, token)
  ...
  const handleMessageSeller = () => {
    if (request && request.userId && request.userDisplayName) {
      openChatPopup(request.userId, request.userDisplayName);
    }
  };
  return (
    <div className="max-w-5xl mx-auto animate-in fade-in duration-500 pb-12 px-4">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-slate-900 mb-6 transition-colors group"
      >
        <ArrowLeft
          size={16}
      ...
      {/*<button
        ...
        className="flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 transition-all active:scale-95 shadow-lg shadow-emerald-200"
      >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">
            {request.brandAndModel || request.category}
          </h1>
          <p className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded font-bold mt-1 w-fit uppercase tracking-wider">
            ID: {request.id}
          </p>
        </div>
        <div
          className={`px-4 py-1.5 rounded-full ring-1 font-bold text-sm uppercase ${getStatusData(request.status).color}`}
        >
          {getStatusData(request.status).label}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {request.imageUrls?.map((url: string, i: number) => (
              <button
                ...
                className={`w-20 h-20 rounded-xl overflow-hidden ring-2 transition-all ${activeImage === url ? "ring-emerald-500 scale-95" : "ring-transparent opacity-60"}`}
              >
                <img
                  src={url}
                  className="w-full h-full object-cover"
        ...
        <div className="space-y-6">
              ...
              <div className="bg-white p-4 rounded-2xl shadow-sm ring-1 ring-slate-200 border-t-4 border-slate-900">
                ...
                {request.status === 0 && (
                  <button
                    ...
                    disabled={isUpdating}
                    className="w-full py-4 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 hover:bg-emerald-700 active:scale-95 disabled:opacity-50"
                  >
                    <CheckCircle size={16} /> Assign to My Route
                  </button>
                )}
                {request.status === 1 && (
                  <button
                    onClick={() => handleAction("status", 2)}
                    disabled={isUpdating}
                    className="w-full py-4 bg-slate-900 text-white rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 hover:bg-black active:scale-95 disabled:opacity-50"
                  >
                    <Truck size={16} /> Confirm Collection
                  </button>
                )}
                {request.status === 2 && (
                  <button
                    onClick={() => handleAction("status", 3)}
                    disabled={isUpdating}
                    className="w-full py-4 bg-blue-600 text-white rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 hover:bg-blue-700 active:scale-95 disabled:opacity-50"
                  >
                    <CheckCircle size={16} /> Finalize Order
                  </button>
                )}
              </div>
            )}
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm ring-1 ring-slate-200">
            <h3 className="text-xs font-black text-slate-400 uppercase border-b pb-3 mb-4 flex gap-2">
            ...
            <div className="grid grid-cols-2 gap-y-6">
              ...
              <div>
                ...
                <p className="font-bold">
                  {["Scrap", "Damaged", "Working"][request.condition]}
                </p>
              </div>
              <div>
                <p className="text-[10px] text-slate-400 font-bold uppercase mb-1">
                ...
                <p className="font-bold flex items-center gap-2">
                  <Zap
                    size={14}
                    className={
                      request.isPoweringOn ? "text-amber-500" : "text-slate-300"
                    }
                  />
                  {request.isPoweringOn ? "Operational" : "Dead"}
                </p>
              </div>
              <div>
                <p className="text-[10px] text-slate-400 font-bold uppercase mb-1">
              ...
              <div className="col-span-2 pt-4 border-t italic text-sm text-slate-600">
                "{request.itemDescription || "No owner notes provided"}"
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm ring-1 ring-slate-200">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-center gap-2 w-full py-4 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase hover:bg-emerald-700 active:scale-95 shadow-md"
            >
            ...
            {request.status > 0 && (
              <>
                <a
                  ...
                  className="flex items-center justify-center gap-2 py-3.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase hover:bg-black active:scale-95"
                >
                  <Phone size={14} /> Call
                </a>
                <a
                  href={`https://wa.me/${contactInfo.phone.replace(/\+/g, "")}?text=Hi%20${contactInfo.name}...`}
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-center gap-2 py-3.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-xl text-[10px] font-black uppercase hover:bg-emerald-100 active:scale-95"
                >
